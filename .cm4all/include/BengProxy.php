...
class BengProxyHandler {
    ...
    function __construct($browserRequest, $requestUri = false) {
        ...
        $curlConnectTimeout = isset($config["curlconnecttimeout"]) ? $config["curlconnecttimeout"] : 5;
    ...
    function isInErrorState() {
        if ($this->_isInErrorState === FALSE) {
            $this->_isInErrorState = $this->response === FALSE || (defined("CURLINFO_RESPONSE_CODE") && isset($this->curlHandle) && curl_getinfo($this->curlHandle, CURLINFO_RESPONSE_CODE) >= 500);
        }
